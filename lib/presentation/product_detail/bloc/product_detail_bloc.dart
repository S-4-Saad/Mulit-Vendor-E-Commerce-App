...
class ProductDetailBloc extends Bloc<ProductDetailEvent, ProductDetailState> {
        ...
        await ApiService.getMethod(
          ...
          executionMethod: (bool success, dynamic responseData) async {
            if (success && responseData != null) {
              try {
                ProductDetail productDetail = _mapResponseToProductDetail(responseData);
                emit(state.copyWith(
                ...
                emit(state.copyWith(
                  ...
                  errorMessage: 'Failed to parse product data: ${e.toString()}',
                ));
              }
            } else {
              emit(state.copyWith(
  ...
  ProductDetail _mapResponseToProductDetail(dynamic responseData) {
    ...
    if (data['product_more_images'] != null) {
      try {
        ...
        if (moreImagesJson is List) {
          productImages = moreImagesJson.map((img) => '$imageBaseUrl${img.toString()}').toList();
        }
      } catch (e) {
    ...
    final originalPrice = double.tryParse(data['product_price']?.toString() ?? '0') ?? 0.0;
    final discountedPrice = double.tryParse(data['product_discounted_price']?.toString() ?? '0') ?? 0.0;
    ...
    final storeCategoryId = int.tryParse(data['store']?['store_category_id']?.toString() ?? '1') ?? 1;
    ...
    if (data['variations'] != null && data['variations'] is List) {
      variations = (data['variations'] as List).map((variation) {
    ...
    if (data['relatedProducts'] != null && data['relatedProducts'] is List) {
      relatedProducts = (data['relatedProducts'] as List).map((relatedProduct) {
        final originalPrice = double.tryParse(relatedProduct['product_price']?.toString() ?? '0') ?? 0.0;
        final discountedPrice = double.tryParse(relatedProduct['product_discounted_price']?.toString() ?? '0') ?? 0.0;
        final discountPercentage = originalPrice > 0 ? ((originalPrice - discountedPrice) / originalPrice) * 100 : 0.0;
        ...
        return RelatedProduct(
          ...
          imageUrl: relatedProduct['product_image'] != null 
              ? '$imageBaseUrl${relatedProduct['product_image']}' 
              : '',
          categoryId: relatedProduct['category_id']?.toString() ?? '',
          categoryName: relatedProduct['category']?['name']?.toString() ?? '',
        );
      }).toList();
    }
    ...
    return ProductDetail(
      ...
      isDeliveryAvailable: true, // Assuming delivery is available
      price: discountedPrice,
      originalPrice: originalPrice,
      ...
      categoryId: data['category']?['id']?.toString() ?? '', // Real server category ID
